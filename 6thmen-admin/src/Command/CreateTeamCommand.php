...
class CreateTeamCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($fileContent as $index => $teamData) {
            ...
            $this->manager->flush();
            $io->success('New team1 created ' . $this->newTeam->getName() . ' from ' . $this->newTeam->getCreatedIn()->format('Y'));
        }
    ...
    private function setLeagues(): ?string
    {
        ...
        foreach (explode('/', $this->rawTeam['league']) as $leagueToFind) {
            ...
            if (!$league) {
                return 'League ' . $leagueToFind . ' does not exist for team1 ' . $this->newTeam->getName() . ' with leagues : ' . $this->rawTeam['league'];
            }
